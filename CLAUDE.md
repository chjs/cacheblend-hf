...
Tests are tagged so CI can run only the cheap subset and developers can opt in to the rest:

- **(no marker)** — fast, no model loading, no GPU. Always run in CI. Examples: `tests/test_smoke.py` (import checks).
- **`@pytest.mark.slow`** — > 30s. Stress / long-context. Skipped by default. Run with `pytest -m slow`.
- **`@pytest.mark.gpu`** — needs CUDA. Skipped on Mac and in CI. Run on vast.ai.
- **`@pytest.mark.requires_model`** — downloads or loads HF weights (e.g. Qwen2.5-1.5B). Skipped in CI to avoid network + multi-GB downloads. Run locally with `pytest -m "requires_model and not slow"`.

CI command (`.github/workflows/ci.yml`): `pytest -v -m "not gpu and not slow and not requires_model"`. Local "everything fast" command: same. Local "everything except GPU/slow": `pytest -v -m "not gpu and not slow"`.
